...
const searchValue = params.get("recherche")
...
$(function () {
    ...
    let displayMovies = function (movies) {
        ...
        movies.forEach((movie) => {
            movieToDisplay += '<div class="d-flex mb-4 border border-2 rounded-2 movie justify-content-between">';
            movieToDisplay += `<div>`;
            movieToDisplay += movie.poster_path ? `<img src="${URL_IMAGE_PREFIX + movie.poster_path}" alt="image-${movie.id}">` : "";
            movieToDisplay += `</div>`;
            movieToDisplay += "<div>";
            movieToDisplay += `<h6><a  href="film.html?filmId=${movie.id}">${movie.original_title}</a></h6>`;
    ...
    onClickFunction();
    $("#search-button").click(onClickFunction);
    displayPagination();
